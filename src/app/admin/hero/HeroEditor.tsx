      ...
      <div className={sectionClass}>
        ...
        {fields.map(({ key, label, multiline }) => (
          <div key={key}>
            ...
            {multiline ? (
              <textarea rows={3} value={data.en[key]} onChange={e => setField("en", key, e.target.value)}
                className={cn(inputClass, "resize-none")} dir="ltr" />
            ) : (
              <input value={data.en[key]} onChange={e => setField("en", key, e.target.value)}
      ...
      <div className={sectionClass}>
        ...
        {fields.map(({ key, label, multiline }) => (
          <div key={key}>
            <label className={labelClass}>{label}</label>
            {multiline ? (
              <textarea rows={3} value={data.ar[key]} onChange={e => setField("ar", key, e.target.value)}
                className={cn(inputClass, "resize-none")} />
            ) : (
              <input value={data.ar[key]} onChange={e => setField("ar", key, e.target.value)}
