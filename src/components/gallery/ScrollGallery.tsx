...
function GalleryCard({
  ...
  /* per-card reactive transforms derived from activeIndex */
  ...
  /* is this card the active (centre) card? */
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between">
          <span className="font-mono text-white/35 text-[11px] font-bold tracking-[0.2em]">
            {numStr}
          </span>
          {product.popular && (
            <div className="flex items-center gap-1 bg-amber-400/18 border border-amber-400/35 rounded-full px-2 py-0.5">
              <Flame className="w-2.5 h-2.5 text-amber-400" />
              <span className="text-amber-300 text-[8px] font-black uppercase tracking-widest">
                Hot
              </span>
            </div>
          )}
        ...
        <div>
          ...
          <div className="flex items-end justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-white/32 text-[9px] mt-0.5 font-medium">
            ...
            <div className="flex-1 p-5 flex flex-col overflow-hidden">
              ...
              <div className="flex items-start gap-3 mb-3 flex-shrink-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <h3 className="text-white font-black text-[13px] leading-tight line-clamp-2">
                    {product.name}
                  </h3>
                </div>
                {product.badge && (
              ...
              {/* description */}
              <p className="text-white/60 text-[10.5px] leading-relaxed mb-3 line-clamp-2 flex-shrink-0">
                {product.description}
              </p>
              ...
              <div className="flex-shrink-0 pt-3 mt-2 border-t border-white/12">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    {product.discount && (
                      <span className="ml-2 text-rose-400 text-[9px] font-black">
                        -{product.discount}%
                      </span>
                    )}
                  </div>
...
export default function ScrollGallery() {
  ...
  const scrollToIndex = useCallback(
    (targetIdx: number) => {
      ...
      window.scrollTo({
        top: el.offsetTop + progress * maxScroll,
        behavior: "smooth",
      });
    },
    []
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 h-screen overflow-hidden bg-black">
        ...
        {/* ── ambient radial glow (shifts with product) ── */}
        <AnimatePresence mode="wait">
          <motion.div
            ...
            style={{
              background: `radial-gradient(ellipse 70% 60% at 50% 55%, rgba(14,165,233,0.07) 0%, transparent 70%)`,
            }}
          />
        </AnimatePresence>
        ...
        <div className="absolute top-8 left-8 z-30 pointer-events-none select-none">
          ...
          <h2 className="text-white font-black leading-[0.92] tracking-tight">
            <span className="block text-[28px] sm:text-[38px] md:text-[46px]">
              MEDICAL PACKAGES
            </span>
            <span className="block text-[28px] sm:text-[38px] md:text-[46px] text-white/30">
              &amp; SERVICES
            </span>
          </h2>
        ...
        {/* ── current card index display (top-right) ── */}
        <div className="absolute top-8 right-8 z-30 text-right pointer-events-none select-none">
          <AnimatePresence mode="wait">
            <motion.p
              key={displayIndex}
        ...
        {/* ── bottom info strip (active card details) ── */}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <p className="text-white/28 text-[8px] font-bold uppercase tracking-[0.22em] mb-1">
                {products[displayIndex]?.category}
              </p>
              ...
              <p className="text-white/55 font-black text-base">
                {products[displayIndex]?.price.split(" / ")[0]}
              </p>
            </motion.div>
          </AnimatePresence>
        </div>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 flex flex-col items-center gap-3">
            ...
            {products.map((_, i) => (
              <motion.button
                ...
                animate={{
                  ...
                  backgroundColor:
                    i === displayIndex ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.18)",
                }}
          ...
          <p className="text-white/22 text-[9px] font-black uppercase tracking-[0.24em]">
            SCROLL TO SURF
          </p>
        </div>

        {/* ── "hover to reveal" hint on active card ── */}
        <AnimatePresence>
          {displayIndex !== undefined && (
            ...
            >
              <p className="text-white/20 text-[9px] font-bold uppercase tracking-[0.22em]">
                HOVER TO REVEAL
              </p>
            </motion.div>
          )}
